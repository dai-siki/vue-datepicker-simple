...
gulp.task('js', function() {
	...
	gulp.src('./example2/demo.js').pipe($.plumber({errorHandler: _errrHandler})).pipe(named(function() {
		return 'demo-src';
	})).pipe(webpack(webpack_config)).pipe($.uglify()).pipe(gulp.dest('./example2'));
});
...
gulp.task('serve', function() {
	...
	gulp.watch([
		'./example/demo.js', './example2/demo.js', './*.vue', './*.css'
	], ['js']);
	gulp.watch(['./scss/*.scss'], ['css']);
	gulp.watch([
		'./example/**/*.html', './example/demo-src.js', './example2/**/*.html', './example2/demo-src.js'
	], function() {
		reload();
	});
});
